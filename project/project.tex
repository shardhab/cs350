...
The overall efficiency of Dual Pivot Quicksort is the same as the classical
Quicksort. According to both Yaroslavskiy\cite{Yaroslavskiy09} and 
Wild\cite{Wild15} (although not the findings of the measurements 
reported in this research paper), Dual Pivot Quicksort is overall faster
than classical Quicksort. However, this is only by a constant factor and
does not significantly alter the efficiency of the algorithm. 
...
advances through the area not yet checked. See Figure~\ref{example-threeway-pivot}. As the algorithm iterates through the array, there are three 
possibilities: if the value at the 
...
\begin{figure}
\List{s2/3, s2/1, s2/2, s3/4, s3/4, s3/4, 6, 3, 4, 9, s1/5, s1/8}
\caption{An array partitioned with dual pivots. The blue elements are
less than the pivot (4). The red elements are equal to the pivot, 
The orange elements are greater than the pivot. The elements not
colored have not been partitioned yet.}
\label{example-threeway-pivot}
\end{figure}
...
Like Quicksort, Three Way Quicksort does not significantly alter the algorithm,
although it does introduce some overhead in keeping track of three partitions
instead of two. Depending upon the input, Three Way Quicksort is either
faster or slower than classical Quicksort, but they are still in the same
overall efficiency class.
...
investigation, we discovered that our implementation of Dual Pivot Quicksort was actually a simplified version of the algorithm created by Yaroslavskiy\cite{Yaroslavskiy09}. His algorithm is
...
numbers is slower than Quicksort, although he reported it to be a faster algorithm. 
...
In particular, Yaroslavskiy's algorithm contains the code found in
Figure~\ref{pseudo-yaroslavskiy}. According to Wild~\cite{Wild15},
the interaction between lines~\ref{yaro1}-\ref{yaro2} and 
lines~\ref{yaro3}-\ref{yaro4} minimizes the comparisons needed to partition the array. 
Our simplified implementation has removed some of these lines, which may
explain why our results are not as good as his.
Given more time, we would have attempted to implement a more complex algorithm closer to
his implementation in order to verify his findings. However, with the time constraints on this
project, we will merely note the problem found here and leave it for future investigation.

\begin{figure}
\begin{algorithmic}[1]
	\While{$k \le g$}
                \If{$A[k] < p$} \label{yaro1}
			\State $swap(A[k], A[l])$
			\State $l \gets l + 1$
 		\Else
			\If{$A[k] \ge q$} \label{yaro2}
				\While{$A[g] > q \text{ and } k < g$} \label{yaro3}
               			 	\State $g \gets g - 1$ \label{yaro4}
                       	 	\EndWhile
				\If{$A[g] > p$}
					\State $swap(A[k], A[g])$
				\Else
	...
	\EndWhile	
\end{algorithmic}
\caption{Partial Pseudocode for Yaroslavskiy's Dual Pivot Quicksort}
\label{pseudo-yaroslavskiy}
\end{figure}
